...
import com.ynov.bibliotech.com.domain.Livre;
import com.ynov.bibliotech.com.repository.LivreRepository;
...
/**
 * REST controller for managing {@link com.ynov.bibliotech.com.domain.Livre}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class LivreResource {
     ...
     */
    @GetMapping("/livres")
    public ResponseEntity<List<Livre>> getAllLivres(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Livres");
        Page<Livre> page;
        if (eagerload) {
            page = livreRepository.findAllWithEagerRelationships(pageable);
        } else {
